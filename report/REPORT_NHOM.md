...
|-----------|----------|-------------|------------|-------------------|
| `utsc-borrowing-policy.md` (5,858 ký tự) | FixedSizeChunker (`fixed_size`) | 39 | 199.0 | Kém: Cắt ngang bảng tra cứu hạn mượn và ranh giới điều khoản phạt. |
| | SentenceChunker (`by_sentences`) | 14 | 416.6 | Khá: Giữ trọn vẹn từng câu nhưng các dòng bảng markdown bị gom cụt lủn. |
| | RecursiveChunker (`recursive`) | 41 | 141.3 | Tốt: Tách tự nhiên theo đoạn `\n\n` rồi đến dòng `\n`, các điều khoản giữ được tính độc lập. |
| `utsc-course-reserves.md` (1,566 ký tự) | FixedSizeChunker (`fixed_size`) | 11 | 188.0 | Kém: Cắt đứt các bước hướng dẫn truy cập Quercus ở giữa câu. |
| | SentenceChunker (`by_sentences`) | 6 | 259.2 | Khá: Giữ câu nguyên vẹn nhưng mất liên kết giữa tiêu đề mục và nội dung con. |
| | RecursiveChunker (`recursive`) | 10 | 154.8 | Rất tốt: Phân tách rõ ràng giữa phần giới thiệu và danh sách các bước thực hiện. |
| `utsc-technology-loans.md` (2,855 ký tự) | FixedSizeChunker (`fixed_size`) | 19 | 197.7 | Kém: Xé đôi thông số kỹ thuật của thiết bị và thời hạn cho mượn. |
| | SentenceChunker (`by_sentences`) | 14 | 202.6 | Trung bình: Gặp khó với các danh sách gạch đầu dòng ngắn (bullet points). |
| | RecursiveChunker (`recursive`) | 20 | 141.4 | Tốt: Bảo toàn cấu trúc từng nhóm thiết bị (Computing, Audio, Power). |
...
- **Metadata filter:** Dùng `category=hours` cho câu giờ mở cửa, `audience=student` cho câu borrowing và proxy borrower, `category=technology` cho câu trả laptop, và `audience=faculty` cho câu TSpace.
- **Code snippet (custom):**
```python
...
- **Cấu hình đã kiểm thử:** Dùng chunk con 250 ký tự với overlap 25 để tìm kiếm; section cha tối đa 800 ký tự để đưa vào ngữ cảnh trả lời; truy xuất `top_k=3`.
...
```
- **Kết quả kiểm thử:** Corpus tạo 54 parent và 118 child. Với Gemini embedding `gemini-embedding-001`, 5/5 query có tài liệu liên quan trong top-3; agent đạt **9/10**. A/B query 4 tăng từ 1/2 khi không lọc lên 2/2 khi lọc `audience=student`.
...
- **Metadata filter:** Áp dụng `metadata_filter={"category": "hours"}` cho câu hỏi thời gian mở cửa thư viện, `metadata_filter={"audience": "student"}` cho câu hỏi hạn mức mượn tài liệu và proxy borrower, `metadata_filter={"category": "technology"}` cho câu hỏi quy trình hoàn trả laptop mượn, và `metadata_filter={"audience": "faculty"}` cho câu hỏi kho lưu trữ nghiên cứu TSpace.
- **Code snippet (custom):**
```python
...
| Từ Hoàng Giang | Custom Parent-child (Child: 250, overlap: 25, Parent: tối đa 800) | 9 / 10 (5/5 query có tài liệu liên quan trong top-3) | Tối ưu hóa kép: vector search chính xác trên chunk con, LLM đọc ngữ cảnh đầy đủ từ chunk cha; 54 parent và 118 child. | Query giờ mở cửa bỏ sót ngoại lệ ngày 12/10. |
...
| 4 | Which form should be completed to designate another borrower when the patron cannot visit the library because of a disability? | The patron should complete the proxy application form to designate another borrower. | `utsc-borrowing-policy` — “Designate a Proxy Borrower” | **A/B test:** không lọc so với `metadata_filter={"audience": "student"}`; tài liệu accessibility chứa đáp án nhiễu “Pickup Authorization Form”. |
| 5 | Which service is a free institutional open-access repository for preserving and disseminating papers produced by faculty and graduate researchers? | TSpace Research Repository. | `utsc-research-publishing` — “Publishing & Repository Platforms” | Định danh dịch vụ theo chức năng, bám đúng mô tả trong corpus. |
...
| 4 | Biểu mẫu chỉ định proxy borrower khi không thể đến thư viện | Parent-child / SemanticChunker (có `audience=student` filter) | Có (Top-1 với Parent-child sau khi lọc) | A/B Parent-child: không lọc trả lẫn hai biểu mẫu và đạt 1/2; lọc `student` chỉ trả “proxy application form” và đạt 2/2. |
...
> - **Khi KHÔNG lọc (`metadata_filter=None`)**: `utsc-accessibility-services#parent-7` đứng top-1 với score 0.8234, còn `utsc-borrowing-policy#parent-6` đứng top-2 với score 0.8227. Agent trộn hai đáp án “Pickup Authorization Form” và “proxy application form”, nên chỉ đạt 1/2.
> - **Khi CÓ lọc (`metadata_filter={"audience": "student"}`)**: `utsc-borrowing-policy#parent-6` lên top-1 với score 0.8227; agent chỉ trả “proxy application form” và đạt 2/2. Điều này chứng minh pre-filtering loại bỏ câu trả lời đúng chủ đề nhưng sai đối tượng.
...
2. **Trường hợp 2 — Câu hỏi 4 (A/B Test khi không dùng filter):**
   - **Hiện tượng lỗi**: Không lọc, top-1 là `utsc-accessibility-services#parent-7` và agent liệt kê cả “Pickup Authorization Form” lẫn “proxy application form”, tạo câu trả lời mâu thuẫn theo đối tượng.
   - **Nguyên nhân**: Hai tài liệu cùng nói về proxy borrower và người không thể đến thư viện, nhưng áp dụng cho metadata audience khác nhau và yêu cầu biểu mẫu khác nhau.
   - **Đề xuất sửa**: Áp dụng tiền lọc `metadata_filter={"audience": "student"}` để chỉ giữ quy định dành cho sinh viên trước khi tính similarity.
